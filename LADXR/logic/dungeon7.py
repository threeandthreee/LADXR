...
class Dungeon7:
    def __init__(self, options, world_setup, r):
        ...
        three_of_a_kind_north = Location(dungeon=7).add(DungeonChest(0x211)).connect(topright_pillar_area, AND(OR(FEATHER, r.hit_switch), r.enemy_requirements["THREE_OF_A_KIND"]))  # compass chest; either hit the switch, or have feather to fall on top of raised blocks. No bracelet because ball does not reset
        ...
        if options.logic == 'glitched' or options.logic == 'hell':
            ...
            final_pillar_area.connect(bottomleftF2_area, AND(r.sideways_block_push, OR(r.enemy_requirements["THREE_OF_A_KIND"], POWER_BRACELET))) # sideways block push to get to the chest and pillar, kill requirement for 3 of a kind enemies to access chest. Assumes you do not get ball stuck on raised pegs for bracelet path
            if options.owlstatues == "both" or options.owlstatues == "dungeon":
